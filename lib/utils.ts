...
const BoxStyles = (element: any) => {
  return {
    ...
    borderWidth: element.borderWidth
      ? convertSizeToPixels(element.borderWidth)
      : "1px",
    ...
    .map((content: any) => {
      switch (content.type) {
        ...
        case "box":
          return `<div style="${convertToKebabCase(
            BoxStyles(content)
          )}">${renderContents(content.contents)}</div>`;
        default:
          return "";
      }
    })
...
export function createHtmlFromJson(json: any) {
  ...
  if (json.hero && json.hero.contents) {
    if (json.hero.contents.length === 0) {
      ...
      if (
        json.hero.contents[0].contents &&
        json.hero.contents[0].contents.length < 1
      ) {
        heroClasses.push("p-4");
      }
      if (
        json.hero.contents[0].contents &&
        json.hero.contents[0].contents[0]?.type === "box"
      ) {
        heroClasses.push("px-4", "py-4");
      }
      if (
        json.hero.contents[0].contents &&
        json.hero.contents[0].contents.length > 0 &&
        json.hero.contents[0].type !== "box"
      ) {
...
export function rewriteFlexHtml(flexHtml: string, size: string): string {
  ...
  const updatedFlexHtml = flexHtml.replace(
    /max-width:\s*\d+px;/,
    `max-width: ${maxWidth};`
  );
...
export const loadOpenCV = () => {
  return new Promise((resolve, reject) => {
    if (typeof window !== "undefined") {
      ...
      script.onload = () => {
        console.log(
          "OpenCV script loaded. Checking for OpenCV initialization..."
        );
...
// logo detection from google api
...
// auto crop from google vision api
...
// gpt api for text extraction and recognition
...
// for autocropping in google vision api
