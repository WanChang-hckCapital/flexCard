...
async function Transactions({
    ...
    const transactionDetailsRangeOption =
        getRangeOption(searchParams.transactionDetailsByDateRange, searchParams.transactionDetailsRangeFrom, searchParams.transactionDetailsRangeTo) ||
        RANGE_OPTIONS.last_7_days

    const transactionStats = await fetchTransactionStats(authenticatedUserId, transactionDetailsRangeOption.startDate, transactionDetailsRangeOption.endDate);
                ...
                <main className="grid flex-1 items-start gap-4 sm:py-0 md:gap-8 lg:grid-cols-3 xl:grid-cols-3">
                    <div className="grid auto-rows-max items-start gap-4 md:gap-4 lg:col-span-2">
                        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3">
                            <Card>
                                <CardHeader className="pb-2">
                                    ...
                                    <CardTitle className="text-[32px]">{totalTransactions}</CardTitle>
                                </CardHeader>
                                <CardContent>
                                ...
                                <CardFooter>
                                    <Progress value={transactionIncreaseRate} aria-label={`${transactionIncreaseRate}% increase`} />
                                </CardFooter>
                            </Card>
                            <Card>
                                <CardHeader className="pb-2">
                                    <CardDescription className="text-slate-300">Total Fees</CardDescription>
                                    <CardTitle className="text-[32px]">${totalFees.toFixed(2)}</CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <div className="text-[12px] text-slate-300">
                                ...
                                <CardFooter>
                                    <Progress value={feeIncreaseRate} aria-label={`${feeIncreaseRate}% increase`} />
                                </CardFooter>
                            </Card>
                            {/* <Card
                        ...
                        <div className="grid gap-4">
                            ...
                            >
                                <TransactionAnalysisByDate userId={authenticatedUserId} startDate={transactionDetailsRangeOption.startDate} endDate={transactionDetailsRangeOption.endDate}/>
                            </ChartCard>
                        </div>
                    </div>
                </main>
