...
import {
  loadForumCommentReply,
  submitForumCommentReply,
      ...
      if (comment.image) {
        try {
          const imageResponse = await fetch(
            `/api/forum-comment-image-load/${comment.image}`
          );
  ...
  useEffect(() => {
    if (comment.replyCount > 0) {
      const fetchReplies = async () => {
        try {
          const repliesResponse = await loadForumCommentReply(comment._id);
    ...
    try {
      ...
      if (imageFile) {
        ...
        const commentReplyImageUploadRes = await fetch(
          "/api/forum-comment-reply-image-submit",
          {
            method: "POST",
            body: formData,
          }
      ...
      const response = await submitForumCommentReply({
