...
function CardEditor({ liveMode, componentId }: Props) {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [componentId, dispatch, liveMode, state.editor.component]);
  ...
  console.log(
    "state.editor.component in Workspacessssss: ",
  ...
  const styles = (currentIndex: number): React.CSSProperties => {
    ...
    if (currentIndex === 0) {
      return {
        width: "400px",
        maxWidth:
          state.editor.component.size === "nano"
            ? "120px"
            : state.editor.component.size === "micro"
            ? "160px"
            : state.editor.component.size === "deca"
            ? "220px"
            : state.editor.component.size === "hecto"
            ? "241px"
            : state.editor.component.size === "kilo"
            ? "260px"
            : state.editor.component.size === "mega"
            ? "300px"
            : state.editor.component.size === "giga"
            ? "386px"
            : "300px",
        marginLeft: "8px",
        marginRight: "8px",
      ...
      return {
        width: "400px",
        maxWidth:
          state.editor.component.contents?.[currentIndex]?.size === "nano"
            ? "120px"
            : state.editor.component.contents?.[currentIndex]?.size === "micro"
            ? "160px"
            : state.editor.component.contents?.[currentIndex]?.size === "deca"
            ? "220px"
            : state.editor.component.contents?.[currentIndex]?.size === "hecto"
            ? "241px"
            : state.editor.component.contents?.[currentIndex]?.size === "kilo"
            ? "260px"
            : state.editor.component.contents?.[currentIndex]?.size === "mega"
            ? "300px"
            : state.editor.component.contents?.[currentIndex]?.size === "giga"
            ? "386px"
            : "300px",
        marginLeft: "8px",
        marginRight: "8px",
  ...
  const renderComponent = (component: EditorComponent, index?: number) => {
    if (component.type === "carousel") {
      ...
      const dynamicStyles =
        component.type === "bubble" ? styles(index ?? 0) : styles;

      return (
        <div
          style={
            typeof dynamicStyles === "function"
              ? dynamicStyles(index ?? 0)
              : dynamicStyles
          }
          onClick={(e) =>
            handleOnClickBody(e, component as EditorElement, component.id || "")
          }>
          {component.hero && (
            <div>
              <AddBoxDropdown
                heroSectionId={component.hero.id}
                bubbleId={component.id || ""}
              />
            </div>
          )}
          ...
          <div
            className={clsx(
              "component-container relative mb-2 border-b border-x rounded-b-lg shadow-lg bg-white overflow-hidden",
              {
                "mt-2":
                  component.hero?.contents &&
                  component.hero?.contents.length < 1 &&
                  component.header?.contents &&
                  component.header?.contents.length < 1,
                "border-t":
                  component.hero?.contents &&
    ...
    <div
      ...
      {state.editor.previewMode && state.editor.liveMode && (
        <Button
          variant="ghost"
          size="icon"
          className="w-32 h-8 bg-slate-600 p-[2px] fixed top-5 right-5 z-[100]"
      ...
      <div className="components-flex-container w-[78%] max-w-fit">
        {state.editor.component ? (
          renderComponent(state.editor.component)
        ) : (
